...
function capitalA(s){
    // Add your logic.
    let letterRe = /[a-zA-Z]*A$/;
    return letterRe.test(s);
...
function imagesSearcher(text) {
    let arr = [];
    // Regular expression to match image names with extensions .jpg, .jpeg, and .png
    let regex = /\b[\w-]+\.(jpg|jpeg|png)\b/gi;
    // Find all matches in the text
    let matches = text.match(regex);
    // Add matched image names to the array
    if (matches) {
        arr = matches;
    }
    return arr;
